        ...
        {!isLoaded ? (
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            <div className="w-full lg:w-[70%] flex flex-col gap-4">
              {cartItems.map((item) => (
                <div key={item.id} className="bg-white dark:bg-[#111] rounded-2xl border border-gray-200 dark:border-gray-800 p-6 flex flex-col sm:flex-row gap-6 shadow-sm">
                  ...
                  <div className="flex-1 flex flex-col justify-center">
                    ...
                    <p className="text-xs text-gray-500 uppercase font-bold tracking-widest mb-4">Ref: {item.details}</p>
                    
                    {item.customization.length > 0 || (item as any).medallionImage ? (
                      <div className="bg-yellow-50 dark:bg-yellow-900/10 p-3 rounded-lg border border-yellow-100 dark:border-yellow-800/30">
                        ...
                        {(item as any).medallionImage && (
                          <div className={`flex items-center gap-2 ${item.customization.length > 0 ? 'mt-3 border-t border-yellow-200 dark:border-yellow-800/30 pt-3' : ''}`}>
                            <span className="text-[10px] font-black uppercase text-yellow-800 dark:text-yellow-600">Medalla Circular:</span>
                            <img src={(item as any).medallionImage} className="w-8 h-8 rounded-full object-cover border border-yellow-500" alt="Medalla" />
                          </div>
                        )}
                      </div>
                    ) : (
                      <p className="text-sm text-gray-400 italic">Sin grabar</p>
                    )}
                  </div>
                  <div className="flex flex-col justify-end items-end sm:items-center sm:justify-center">
                    {(item as any).isOffer && (
                      <span className="text-gray-400 line-through text-sm font-bold">Q{(item as any).originalPrice?.toFixed(2)}</span>
                    )}
                    <span className="text-2xl font-black text-[#d32f2f]">Q{item.price.toFixed(2)}</span>
                  </div>
                </div>
              ))}
            </div>
